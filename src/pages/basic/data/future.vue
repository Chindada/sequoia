...
const items = ref<FutureDetailList>(FutureDetailList.create());
const selectedItems = ref<FutureDetailList>(FutureDetailList.create());
...
onMounted(async () => {
  document.title = `TBT Capitan - ${t("future")}${t("basic_data")}`;
});

const loadItems = async () => {
  await BasicAPI.GetAllFutureDetail()
    .then((res) => {
      items.value = res;
    })
    .catch((e: APIResponse) => {
...
<template>
  <div class="card">
    <DataTable
      ...
      "
      data-key="code"
      :export-filename="'Capitan_'.concat(t('future'))"
      :filters="filters"
      paginator
