...
function onPresetChange() {
  ...
  let presetValue;
  if (preset.value === "Aura") {
    presetValue = Aura;
  } else if (preset.value === "Lara") {
    presetValue = Lara;
  } else if (preset.value === "Nora") {
    presetValue = Nora;
  }
...
<template>
  <div
    class="config-panel bg-surface-0 dark:bg-surface-900 border-surface rounded-border absolute top-[3.25rem] right-0 hidden w-64 origin-top border p-4 shadow-[0px_3px_5px_rgba(0,0,0,0.02),0px_0px_2px_rgba(0,0,0,0.05),0px_1px_4px_rgba(0,0,0,0.08)]"
  >
      ...
      <div>
        ...
        <div class="flex flex-wrap justify-between gap-2 pt-2">
          <button
            ...
            :class="[
              'h-5 w-5 cursor-pointer rounded-full border-none p-0 outline-offset-1 outline-none',
